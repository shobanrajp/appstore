...
async def verify_payment(verify_data: RazorpayVerifyPayment, user: dict = Depends(get_current_user)):
    # Get payment record
    payment = await db.payments.find_one({"id": verify_data.payment_id, "user_id": user["id"]})
    if not payment:
        raise HTTPException(status_code=404, detail="Payment not found")
    
    # Get store credentials for verification
    store = await db.stores.find_one({"id": payment["store_id"]}, {"_id": 0})
    if not store:
        raise HTTPException(status_code=404, detail="Store not found")
...
async def create_default_admin():
    # In serverless (e.g., Vercel), avoid hard-failing cold starts on DB issues
    if os.getenv("DISABLE_STARTUP_DB_INIT", "true").lower() in ("1", "true", "yes"): 
        logger.info("Skipping startup DB init (DISABLE_STARTUP_DB_INIT=true)")
        return
    try:
        ...
        logger.warning(f"Startup DB init skipped due to error: {e}")
